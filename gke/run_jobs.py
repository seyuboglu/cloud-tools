...
def load_base_job_config():
    return yaml.load(open('gke/job.yaml'), Loader=yaml.FullLoader)
...
def create_job_config(job_key, job_id, method_to_run, file_to_run, config_path, store_path, use_gdb, gpu, pool):
    ...
    startup_cmds = startup_commands(method_to_run, file_to_run, config_path, use_gdb)
    ...
    if pool is not None:
        base_config['spec']['template']['spec']['nodeSelector']['cloud.google.com/gke-nodepool'] = f'gpu-pool-{pool}'
